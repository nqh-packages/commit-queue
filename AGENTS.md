...
| Do not bypass the qlty pre-commit gate in this repo            | qlty is the repo-level lint, security, smell, and maintainability adapter |
...
| Qlty CLI wrapper     | `scripts/run-qlty.sh`                         |
| Native behavior gate | `npm test`                                    |
| Combined local gate  | `npm run quality`                             |
| Commit gate          | `.githooks/pre-commit` runs `npm run quality` |

Qlty is required for this repo. Install with `curl https://qlty.sh | bash` if
