...
test("local installer wires commit-queue into common shell startup files", () => {
  ...
  try {
    ...
    assert.equal(
      JSON.parse(
        readFileSync(path.join(home, ".commit-queue", "package.json"), "utf8"),
      ).type,
      "module",
    );
...
test("local installer keeps commit-queue first without duplicating PATH entries", () => {
  ...
  try {
    ...
    for (const { shell, args } of checks) {
      ...
      assert.equal(
        pathParts.filter(
          (entry) => entry === path.join(home, ".commit-queue", "bin"),
        ).length,
        1,
...
test("local installer wires commit-queue repo hooks into Git config", () => {
  ...
  try {
    ...
    const hooksPath = spawnSync(
      "/usr/bin/git",
      ["-C", hookRepo, "config", "--local", "--get", "core.hooksPath"],
      {
        encoding: "utf8",
      },
    );
...
test("refresh script installs committed HEAD and ignores dirty working tree edits", () => {
  ...
  const sourceRepo = mkdtempSync(
    path.join(tmpdir(), "commit-queue-refresh-repo-"),
  );
  try {
    cpSync(repoRoot, sourceRepo, {
      recursive: true,
      filter: (source) =>
        ![".git", "node_modules", "dist", "coverage"].some(
          (name) =>
            source === path.join(repoRoot, name) ||
            source.startsWith(`${path.join(repoRoot, name)}${path.sep}`),
        ),
    });
    assert.equal(
      spawnSync("/usr/bin/git", ["init", "-b", "main"], {
        cwd: sourceRepo,
        encoding: "utf8",
      }).status,
      0,
    ...
    assert.equal(
      spawnSync("/usr/bin/git", ["config", "user.name", "Commit Queue Test"], {
        cwd: sourceRepo,
      }).status,
      0,
    );
    assert.equal(
      spawnSync(
        "/usr/bin/git",
        ["config", "user.email", "commit-queue@example.test"],
        { cwd: sourceRepo },
      ).status,
      0,
    );
    assert.equal(
      spawnSync("/usr/bin/git", ["add", "."], { cwd: sourceRepo }).status,
      0,
    );
    assert.equal(
      spawnSync("/usr/bin/git", ["commit", "-m", "test: source"], {
        cwd: sourceRepo,
        encoding: "utf8",
      }).status,
      0,
    ...
    const result = spawnSync(
      process.execPath,
      [
        path.join(sourceRepo, "scripts", "refresh-installed-runtime.js"),
        "--hook",
        "test",
      ],
      {
    ...
    assert.doesNotMatch(
      readFileSync(path.join(home, ".commit-queue", "bin", "git"), "utf8"),
      /dirty hook leak/,
    );
    ...
    const marker = path.join(
      ...
      ".commit-queue",
      "stale-installs",
      `${createHash("sha256").update(sourceRepo).digest("hex").slice(0, 24)}.json`,
    );
