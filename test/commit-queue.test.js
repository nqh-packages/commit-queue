...
test("read-only commands pass through without a session", () => {
  ...
  try {
    ...
    const log = runCommitQueue(fixture.repo, ["log", "-1", "--pretty=%s"], {
      state: fixture.state,
    });
    const branch = runCommitQueue(fixture.repo, ["branch", "--show-current"], {
      state: fixture.state,
    });
    const lsFiles = runCommitQueue(fixture.repo, ["ls-files", "README.md"], {
      state: fixture.state,
    });
    const config = runCommitQueue(
      fixture.repo,
...
test("outside Git repos, getID is blocked", () => {
  ...
  try {
    const getId = runCommitQueue(temp.root, ["getID"], {
      state: path.join(temp.root, "state"),
    });
...
test("git getID creates a shell-activatable session", () => {
  ...
  try {
    ...
    assert.match(
      result.stdout,
      /export COMMIT_QUEUE_AGENT_SESSION="codex-019da855-918f-7880-a76d-8f1937136f86"/,
    );
...
test("git getID explains why explicit agent session alone is not enough", () => {
  ...
  try {
    ...
    assert.match(
      result.stderr,
      /COMMIT_QUEUE_AGENT_SESSION alone is not enough/,
    );
...
test("protected mutations are blocked while installed runtime refresh marker exists", () => {
  ...
  try {
    ...
    writeFileSync(
      path.join(
        markerDir,
        `${createHash("sha256").update(fixture.repo).digest("hex").slice(0, 24)}.json`,
      ),
      JSON.stringify({
        repo: fixture.repo,
        head: "abc123",
...
test("mutating commands are protected when repo is selected with global Git options", () => {
  ...
  try {
    ...
    const fromDashC = runCommitQueue(
      outside.root,
      ["-C", fixture.repo, "add", "src/a.ts"],
      {
        state: fixture.state,
      },
    );
    const fromGitDir = runCommitQueue(
      outside.root,
      [
...
test("unowned Git commands pass through without a session", () => {
  const fixture = createFixture();
  try {
    const tag = runCommitQueue(fixture.repo, ["tag", "v1.0.0"], {
      state: fixture.state,
    });
    const branch = runCommitQueue(fixture.repo, ["branch", "new-branch"], {
      state: fixture.state,
    });
    const config = runCommitQueue(
      fixture.repo,
    ...
    assert.equal(
      runRealGit(fixture.repo, ["tag", "--list", "v1.0.0"]).stdout.trim(),
      "v1.0.0",
    );
    assert.equal(
      runRealGit(fixture.repo, [
    ...
    assert.equal(
      runRealGit(fixture.repo, ["config", "--get", "alias.co"]).stdout.trim(),
      "checkout",
    );
  } finally {
    fixture.cleanup();
...
test("hook config writes are blocked while harmless config writes pass through", () => {
  const fixture = createFixture();
  try {
    const safeWrite = runCommitQueue(
      fixture.repo,
      ["config", "alias.co", "checkout"],
      { state: fixture.state },
    );
    const hookPathWrite = runCommitQueue(
      fixture.repo,
      ["config", "core.hooksPath", "/dev/null"],
      { state: fixture.state },
    );
    const hookCommandWrite = runCommitQueue(
      fixture.repo,
      ["config", "hook.lint.command", "npm test"],
      { state: fixture.state },
    );
    const hookEventWrite = runCommitQueue(
      fixture.repo,
      ["config", "--add", "hook.lint.event", "pre-commit"],
      { state: fixture.state },
    );
    const hookEdit = runCommitQueue(fixture.repo, ["config", "--edit"], {
      state: fixture.state,
    });
    const hookRead = runCommitQueue(
      fixture.repo,
    ...
    assert.match(
      hookPathWrite.stderr,
      /COMMIT_QUEUE_HOOK_CONFIG_MUTATION_BLOCKED/,
    );
    assert.match(
      hookCommandWrite.stderr,
      /COMMIT_QUEUE_HOOK_CONFIG_MUTATION_BLOCKED/,
    );
    assert.match(
      hookEventWrite.stderr,
      /COMMIT_QUEUE_HOOK_CONFIG_MUTATION_BLOCKED/,
    );
    ...
    assert.equal(
      runRealGit(fixture.repo, ["config", "--get", "core.hooksPath"], {
        allowFailure: true,
      }).stdout.trim(),
      "",
    );
...
test("protected git blocks config-based hook bypass before commit", () => {
  ...
  try {
    ...
    assert.equal(
      runRealGit(fixture.repo, ["config", "core.hooksPath", ".githooks"])
        .status,
      0,
    );
...
test("clone passes through from inside a protected repo, including harmless inline config", () => {
  ...
  try {
    ...
    const clone = runCommitQueue(
      fixture.repo,
      ["clone", "--depth=1", fixture.repo, cloneTarget],
      {
        state: fixture.state,
      },
    );
    const ghStyleClone = runCommitQueue(
      fixture.repo,
      [
        "-c",
        "protocol.version=2",
...
test("push passes through without a session", () => {
  ...
  try {
    ...
    assert.equal(
      runRealGit(fixture.repo, ["remote", "add", "origin", remote.root]).status,
      0,
    );

    const result = runCommitQueue(fixture.repo, ["push", "origin", "main"], {
      state: fixture.state,
    });

    assert.equal(result.status, 0, result.stderr);
    assert.match(
      runRealGit(remote.root, ["rev-parse", "--verify", "main"]).stdout.trim(),
      /^[0-9a-f]{40}$/,
    );
  } finally {
    fixture.cleanup();
...
test("unowned Git commands pass through with a session", () => {
  ...
  try {
    ...
    assert.equal(result.status, 0, result.stderr);
    assert.equal(
      runRealGit(fixture.repo, ["tag", "--list", "v1.0.0"], {
        env,
      }).stdout.trim(),
      "v1.0.0",
    );
...
test("explicit add uses the session index and leaves the shared index clean", () => {
  ...
  try {
    ...
    const sharedIndex = runRealGit(fixture.repo, [
      "diff",
      "--cached",
      "--name-only",
    ]);
...
test("explicit add stages a directory symlink as one path", () => {
  ...
  try {
    ...
    assert.equal(
      runRealGit(fixture.repo, ["add", "apps/booknow/convex"]).status,
      0,
    );
    assert.equal(
      runRealGit(fixture.repo, ["commit", "-m", "test: add symlink"]).status,
      0,
    );
    ...
    assert.match(
      stagedEntry.stdout.trim(),
      /^120000 [0-9a-f]{40} 0\tapps\/booknow\/convex$/,
    );
  } finally {
    fixture.cleanup();
  }
...
test("explicit add from a subdirectory resolves paths like real Git", () => {
  ...
  try {
    ...
    writeRepoFile(
      fixture.repo,
      "apps/booknow/Sources/Screens/BookingDetailScreen.swift",
      "struct Screen {}\n",
    );

    const result = runCommitQueue(
      appDir,
      ["add", "Sources/Screens/BookingDetailScreen.swift"],
      {
        state: fixture.state,
        env,
      },
...
test("explicit add accepts literal bracketed route segments", () => {
  ...
  try {
    ...
    const escapedRoutePathspec =
      "apps/ngoquochuy/src/pages/\\[locale\\]/index.astro";
    writeRepoFile(
      fixture.repo,
      routeFile,
    ...
    const escapedResult = runCommitQueue(
      fixture.repo,
      ["add", escapedRoutePathspec],
      {
        state: fixture.state,
        env,
      },
...
test("git show can read paths from the active session index", () => {
  ...
  try {
    ...
    assert.equal(
      runRealGit(fixture.repo, ["show", ":src/a.ts"], { allowFailure: true })
        .status,
      128,
    );
  } finally {
...
test("clean session commit creates a real commit without polluting the shared index", () => {
  ...
  try {
    ...
    assert.match(
      message,
      new RegExp(`Commit-Queue-Session: ${env.COMMIT_QUEUE_ID}`),
    );
    assert.match(message, /Coding-Agent: codex/);
    assert.match(message, /Coding-Agent-Session: codex-test-session/);
    assert.equal(
      runRealGit(fixture.repo, ["status", "--short"]).stdout.trim(),
      "",
    );
  ...
  try {
    ...
    for (const args of [
      ...
      [
        ...
        "-m",
        "test: spoof",
        "--trailer=Commit-Queue-Session: cq_fake",
      ],
      [
        "commit",
        "-m",
        "test: spoof",
        "--trailer",
        "coding-agent-session=codex-fake",
      ],
    ]) {
      const result = runCommitQueue(fixture.repo, args, {
        state: fixture.state,
...
test("human bypass commits from the raw Git index and strips the secret line", () => {
  const fixture = createFixture();
  try {
    ...
    const events = readFileSync(
      path.join(fixture.state, "logs", "events.jsonl"),
      "utf8",
    );
...
test("human bypass only matches a standalone secret line", () => {
  const fixture = createFixture();
  try {
...
test("human bypass supports Git message files from GUI clients", () => {
  const fixture = createFixture();
  try {
    const secret = "local gui bypass phrase";
    writeHumanBypassConfig(fixture.state, secret);
    ...
    for (const [fileName, args] of [
      ...
      assert.equal(
        runRealGit(fixture.repo, ["add", `src/${fileName}.ts`]).status,
        0,
      );
      writeRepoFile(
        fixture.repo,
        fileName,
        `test: gui file commit ${fileName}\n${secret}\n`,
      );
...
test("human bypass rejects inline secrets in Git message files", () => {
  const fixture = createFixture();
  try {
    writeHumanBypassConfig(fixture.state, "local gui bypass phrase");
    writeRepoFile(fixture.repo, "src/a.ts", "export const a = 1;\n");
    ...
    writeRepoFile(
      fixture.repo,
      "message.txt",
      "test: gui file commit with local gui bypass phrase inline\n",
    );

    const result = runCommitQueue(
      fixture.repo,
      ["commit", "--no-verify", "-F", "message.txt"],
      {
        state: fixture.state,
      },
    );
...
test("human bypass skips protected commit policy instead of partially rechecking it", () => {
  const fixture = createFixture();
  try {
    const secret = "local gui bypass phrase";
    writeHumanBypassConfig(fixture.state, secret);
    ...
    assert.equal(result.status, 0, result.stderr);
    assert.doesNotMatch(result.stderr, /COMMIT_QUEUE_/);
    const message = runRealGit(fixture.repo, [
      "log",
      "-1",
      "--format=%B",
    ]).stdout;
    assert.match(message, /test: raw human path/);
    assert.match(message, /Coding-Agent: human/);
    assert.doesNotMatch(message, /local gui bypass phrase/);
  } finally {
    fixture.cleanup();
  }
...
test("commit --no-post-rewrite is blocked as a hook bypass", () => {
  ...
  try {
    ...
    const result = runCommitQueue(
      fixture.repo,
      ["commit", "--no-post-rewrite", "-m", "test: skip hook"],
      {
        state: fixture.state,
        env,
      },
...
test("commit with hook-path config override is blocked", () => {
  ...
  try {
    ...
    const result = runCommitQueue(
      fixture.repo,
      ["-c", "core.hooksPath=/dev/null", "commit", "-m", "test: bypass hooks"],
...
test("commit blocks sessions without coding agent metadata", () => {
  ...
  try {
    ...
    const result = runCommitQueue(
      fixture.repo,
      ["commit", "-m", "test: missing attribution"],
      {
        state: fixture.state,
        env,
      },
...
test("commit blocks when session metadata points at another session index", () => {
  ...
  try {
    ...
    const firstSessionPath = path.join(
      fixture.state,
      "sessions",
      `${firstEnv.COMMIT_QUEUE_ID}.json`,
    );
    const secondSessionPath = path.join(
      fixture.state,
      "sessions",
      `${secondEnv.COMMIT_QUEUE_ID}.json`,
    );
...
test("commit lock timeout reports the active owner instead of requiring manual deletion", () => {
  ...
  try {
    ...
    assert.match(
      commit.stderr,
      new RegExp(`Active lock owner pid: ${process.pid}`),
    );
...
test("commit blocks when symbolic HEAD changed after session creation", () => {
  ...
  try {
    ...
    assert.equal(
      runRealGit(fixture.repo, ["branch", "--show-current"]).stdout.trim(),
      "same-head",
    );
    assert.equal(
      runRealGit(fixture.repo, ["log", "-1", "--pretty=%s"]).stdout.trim(),
...
test("repo opt-out passes through to real Git", () => {
  ...
  try {
    writeFileSync(
      path.join(fixture.repo, ".commit-queue.json"),
      '{ "enabled": false }\n',
    );
    writeRepoFile(fixture.repo, "src/a.ts", "export const a = 1;\n");
    ...
    assert.equal(result.status, 0, result.stderr);
    assert.match(
      runRealGit(fixture.repo, ["diff", "--cached", "--name-only"]).stdout,
      /src\/a\.ts/,
    );
  } finally {
    fixture.cleanup();
...
test("human Git passthrough is blocked without an interactive terminal", () => {
  ...
  try {
    ...
    const result = runHgit(fixture.repo, ["add", "."], {
      state: fixture.state,
    });
...
function writeHumanBypassConfig(state, phrase) {
  ...
  writeFileSync(
    path.join(state, ".runtime-local.json"),
    `${JSON.stringify(
      {
        humanNoVerifyPhraseHash: createHash("sha256")
          .update(phrase)
          .digest("hex"),
      },
      null,
