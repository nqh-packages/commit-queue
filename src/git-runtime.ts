...
export function runGit(realGit: string, args: string[], options: GitRunOptions = {}): GitResult {
...
export function resolveRepo(realGit: string, globalArgs: string[] = []): string | null {
  const result = runGit(realGit, [...globalArgs, "rev-parse", "--show-toplevel"]);
...
export function isRepoOptedOut(repo: string): boolean {
  ...
  try {
    const config = JSON.parse(readFileSync(configPath, "utf8")) as { enabled?: boolean };
...
export function listStagedPaths(realGit: string, repo: string, indexPath: string): string[] {
...
export function stagedBlob(realGit: string, repo: string, indexPath: string, relativePath: string): string | null {
...
export function worktreeBlob(realGit: string, repo: string, relativePath: string): string | null {
