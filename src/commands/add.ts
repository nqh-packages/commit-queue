...
import { listStagedPaths, resolveGitPathBase, runGit, stagedBlob } from "../git-runtime.js";
import { requireSession } from "../session-guard.js";
import { saveSession } from "../session-store.js";
import type { StagedPath } from "../types.js";

export function handleAdd(realGit: string, repo: string, args: string[], globalArgs: string[] = []): void {
  ...
  if (hasBroadAdd(args)) {
    fail(errorPayload({
      ...
      context: { command: "add", args, repo, cwd: pathBaseCwd, session: session.id },
  ...
  if (unsafePathspec) {
    fail(errorPayload({
      ...
      context: { command: "add", args, repo, cwd: pathBaseCwd, session: session.id, unsafe_pathspec: unsafePathspec },
      suggestions: ["Use `git add path/to/file` for each file you intend to commit."],
      retriable: true,
    }));
  }
  ...
  if (add.status !== 0) {
    if (isPathspecNotFound(add.stderr)) {
      fail(errorPayload({
        ...
        detail: "Git could not find one or more explicit paths for protected staging.",
        context: {
          command: "add",
          args,
          pathspecs: pathArgs,
