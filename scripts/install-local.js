...
const hookRepo = path.resolve(
  process.env.COMMIT_QUEUE_REPO_HOOKS_TARGET || repoRoot,
);
...
function ensureShellProfile(profilePath) {
  const current = existsSync(profilePath)
...
function ensureRepoHooksPath() {
  ...
  const result = spawnSync(
    resolveRealGit(),
    ["-C", hookRepo, "config", "--local", "core.hooksPath", ".githooks"],
    {
      encoding: "utf8",
    },
  );
...
function clearFailedRefreshMarker(repo) {
  rmSync(
    path.join(stateRoot, "stale-installs", `${hash(path.resolve(repo))}.json`),
    { force: true },
  );
}
